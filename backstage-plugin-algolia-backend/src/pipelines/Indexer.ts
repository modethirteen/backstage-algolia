...
export class Indexer extends Writable {
  ...
  public async index(objects: IndexObject[]): Promise<void> {
    ...
    const filteredObjects = objects.map(o => {
      ...
      let { text } = o;
      if (this.chunk) {
        const chunkedObjects: IndexObjectWithIdAndTimestamp[] = []
        let chunks = 0;
        while (text.length) {
          ...
          chunkedObjects.push(this.newIndexObjectWithIdAndTimestamp(location + chunks, o));
        }
        return chunkedObjects;
      }
      const object = this.newIndexObjectWithIdAndTimestamp(location, o);
      const length = Buffer.from(JSON.stringify(object), 'utf-8').length;
      if (length <= this.maxObjectSizeBytes) {
        return [object];
      }
      this.logger.debug(`Object ${object.objectID} with location ${location} is ${length} bytes and larger than the maximum allowed length of ${this.maxObjectSizeBytes} bytes`);
      return [undefined];
    })
    .flat()
    .filter(o => o) as IndexObjectWithIdAndTimestamp[];
