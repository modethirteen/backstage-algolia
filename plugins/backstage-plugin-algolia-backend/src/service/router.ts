...
export interface RouterOptions {
  ...
  queryResultsHandler?: (results: SearchResponse<IndexObjectWithIdAndTimestamp>[], options?: SearchProxyRequest['options']) => Promise<SearchResponse<IndexObjectWithIdAndTimestamp>[]>;
}
...
export async function createRouter(
  ...
  router.post('/pipelines', validateData(pipelinesRequestSchema), async (req, res) => {
  ...
  router.post('/insights', validateData(insightsProxyRequestSchema), async (req, res) => {
    const { insightsMethod, payload, authenticatedUserToken, userToken }: InsightsProxyRequest = req.body;
  ...
  router.post('/query', validateData(searchProxyRequestSchema), async (req, res) => {
    ...
    res.json(queryResultsHandler ? (await queryResultsHandler(response.results as SearchResponse<IndexObjectWithIdAndTimestamp>[], options)) : response);
  });
