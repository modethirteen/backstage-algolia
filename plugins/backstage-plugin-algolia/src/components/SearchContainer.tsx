...
export const SearchContainer = (
  ...
  const { children, authenticatedUserToken, userToken, ...rest } = props;
  ...
  switch (
    config.getOptional<Config['algolia']['insights']>('algolia.insights') ??
    'none'
  ) {
    case 'backend':
      insights = {
  ...
  return (
    <AlgoliaQueryIdContext.Provider value={queryIdProvider}>
      <InstantSearch {...rest} insights={insights}>
        {children}
      </InstantSearch>
